...
object DevAssets extends Controller {
   ...
   */
  def at(rootPath: String, file: String, prodpath:String): Action[AnyContent] = {

    val prod = controllers.Assets.at(path = prodpath, file)

    Play.mode match {
      case Mode.Prod => prod
      case _ => {

        val mode = Play.configuration.getString("devmode", Some(Set("dev", "prod"))).getOrElse("prod");
